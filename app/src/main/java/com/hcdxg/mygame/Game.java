...
public class Game extends GameView
{
	...
	public Game(){
		aabb.lowerBound.set(-100,-100);//最低高度
		aabb.upperBound.set(100,100);//最高高度
		world=new World(G);
		world.setContactListener(cl);//碰撞监听
		blocklist=new Vector<Block>();
		body_remove=new Vector<Body>();//刚体消除
		caidan=false;
		...
		back.setbitmap(tool.loadbitmap("back.png"));//返回页面
